...
- `public/cuedesk-mark.svg` is the **wingscript** mark under a legacy filename (content updated in the rebrand). It is still used for inline `<img>`s (nav/footer on the landing page, legal-page headers) — do not "fix" references to it without renaming the file everywhere. It is **no longer the favicon**: it is non-square (217×256), which made Google reject it and fall back to the old CueDesk `logo192.png` in search results. Favicons are now the square `favicon-48.png`/`favicon-32.png` (white mark on `#5400D1`, generated with `logo192.png`/`logo512.png` from `wingscript-mark-white.png`), referenced with a `?v=ws2` cache-buster
